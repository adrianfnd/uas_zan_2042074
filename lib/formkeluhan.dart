...
class keluhan extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyHomePage(title: 'Data Keluhan'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
        ...
        child: new Center(
            child: new FutureBuilder(
              ...
              builder: (context, snapshot) {
                return keluhanList.length > 0? new KeluhanList(keluhan: keluhanList):
                new Center(child:
                new Text('', style: Theme.of(context).textTheme.headline6));
              },
            )
        ),
      ),
  ...
  Future loadList() {
    final Future futureDB = dbkeluh.initDB();
    return futureDB.then((db) {
      Future<List<Keluhan>> futureKeluhan = dbkeluh.keluhan();
      futureKeluhan.then((keluhanList) {
        setState(() {
